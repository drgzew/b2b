...
def extract_title_parts(full_title):
    ...
    if ' = ' in full_title:
        ...
        # Если нет явного разделения, пытаемся найти английский текст в скобках или после двоеточия
        # Но в большинстве случаев разделение есть
        title_ru = full_title.strip()
        title_en = ""
    
    # Очищаем от суффиксов типа ": выпускная квалификационная работа..."
    # Но сохраняем основное название
    if title_ru:
        # Убираем часть после двоеточия, если там идет описание типа ВКР
        # Но оставляем, если это часть названия
        for suffix in [': выпускная квалификационная работа', ': выпускная квалификационная работа бакалавра', 
                       ': магистерская диссертация', ': дипломная работа']:
...
def extract_institute_and_major(title_text):
    ...
    major_patterns = [
        r'направление\s+([0-9]{2}\.[0-9]{2}\.[0-9]{2}\s+[^,]+?(?::|,|\.|\s*—))',
        r'направление\s+([0-9]{2}\.[0-9]{2}\.[0-9]{2}\s+[^,]+?)(?:,|\.|\s*—)',
        r'специальность\s+([0-9]{2}\.[0-9]{2}\.[0-9]{2}\s+[^,]+?(?:,|\.|\s*—))',
        r'профиль\s+["\']([^"\']+)["\']',
        r'программа\s+["\']([^"\']+)["\']',
    ]
...
def parse_detail_page(detail_url):
    ...
    if title_tag:
        ...
        # Извлекаем авторов
        # Авторы обычно в начале до точки
        if '. ' in full_title:
            author_part = full_title.split('. ', 1)[0]
            # Очищаем от лишнего
            author_part = re.sub(r'^[^,]+,\s*', '', author_part)  # Убираем "Фамилия, " если есть
            
            # Разбиваем авторов
            if ';' in author_part:
                data["authors"] = [a.strip() for a in author_part.split(';') if a.strip()]
            elif ',' in author_part and ' ' in author_part:
                # Формат "Фамилия, Имя Отчество"
                data["authors"] = [author_part]
            else:
                data["authors"] = [author_part]
