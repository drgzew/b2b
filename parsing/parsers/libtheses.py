...
def extract_title_parts(full_title):
    ...
    if '. ' in full_title:
        # Разделяем на часть с авторами и остальное
    ...
    # Теперь ищем разделение по " = " для русской и английской версии
    if ' = ' in title_part:
        ...
        # Ищем конец английского названия - обычно это двоеточие или запятая перед описанием ВКР
        # Паттерн: английское название заканчивается перед ": выпускная" или ", выпускная"
        suffixes = [
        ...
        if not title_en:
            # Ищем конец английского названия перед ": направление" или ", направление"
            match = re.search(r'([^:,]+?)\s*[,:]\s*направление', title_en_part, re.IGNORECASE)
            if match:
                title_en = match.group(1).strip()
            else:
                # Если ничего не нашли, берем всю строку
