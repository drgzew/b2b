...
def login_via_tumgu_email(
    ...
    отдельного личного кабинета и самостоятельного логина для авторов в
    пилоте нет (см. открытый вопрос в docs/api-contract.md). Реальный SSO
    подключается заменой тела parse_tumgu_profile(), сам эндпоинт не меняется.
    """
...
def get_author(
    ...
    Партнёру (role == 'partner') отдаём урезанный набор полей: без email и
    даты рождения, только то, что нужно для решения о стажировке/НИОКР.
    Формально response_model один и тот же (AuthorRead), но для партнёра
    чувствительные поля обнуляются — так фронту не нужно дёргать два разных
    эндпоинта в зависимости от роли.
    """
